...
class date {
  ...
  int name;
  date(int num, int heavy, int name, int weight) {
    this.num=num;
    this.heavy=heavy;
    this.weight=weight;
    this.name = name;
...
class readDate {
 private date[] list = new date[271];
 ...
 void loadDate(String filename) {
   try {
     ...
     while ((line = reader.readLine())!=null) {
       ...
       else {na = 0; System.out.println(i +"無効なデータ");System.exit(0);}
       weight = Integer.parseInt(sp[4]);
       if(num > 18 || heavy > 57 || heavy < 49 || weight > 650 || weight < 350){
         System.out.println(i +"無効なデータ"); System.exit(0);
       }
       list[i] = new date(num,heavy,na,weight);
       time[i] = ti;
       i++;
       y++;
